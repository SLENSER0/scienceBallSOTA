...
## Агентный слой (2026-07, «агенты везде», live на :3000)

По запросу «сделай агентную систему, не жалей доллары на OpenRouter» — фичи, где
платформа не ищет, а РАССУЖДАЕТ, на платных OSS-моделях (glm-5.2 / deepseek-v4-flash).

**«Советник» (Agentic Technology Advisor).** По условиям (материал+гео+числа) ретривит
кандидаты-технологии и запускает **отдельного reasoning-агента на каждого** (glm-5.2,
параллельно, `as_completed`) — оценивает соответствие 0–100 строго по фактам графа
(за/ограничения/пробелы); агент-синтезатор (deepseek) пишет ранжированную рекомендацию,
помечая противоречия. Стриминг SSE — карточки появляются по мере готовности агентов.
`advisor.py` + `POST /advise` + `GET /advise/stream` + `AdvisorView`.

**Валидационный workflow (multi-agent).** 8 реальных горных сценариев × (run advisor →
adversarial judge) + синтез. Вердикт «С ОГОВОРКАМИ», нашёл 3 дефекта: (HIGH) «флотация»
ранжировалась ПОСЛЕДНЕЙ под off-topic пирометаллургией; (MED) валидный мембранный метод
тонул; (LOW) «95% подтверждено» при «данных нет». **Починил:** relevance-тиринг
(on-topic всегда выше off-topic, потом по fit — «флотация» теперь #1) + жёсткий промпт
(нельзя звать число «подтверждённым» без факта графа; cap fit ≤40 data-poor / ≤15 off-topic).

**«Противоречия» (Agentic Arbiter).** Где источники расходятся: агент-арбитр (glm-5.2)
читает провенанс каждой стороны (значение, гео, год, цитата) и выносит вердикт genuine /
context_dependent / resolved / insufficient + рекомендацию. `contradiction_analysis.py` +
`/arbiter/*` + `ContradictionsView`. Проверено вживую: на ложном конфликте «concentration
6 vs 40» агент ПРОЧИТАЛ цитату и распознал, что это мм (апертура спектрометра / размер
образца), а не концентрация — поймал артефакт извлечения рассуждением.
