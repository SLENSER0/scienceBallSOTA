...
# In-process кэш матрицы эмбеддингов узлов: db_path -> запись (сигнатура = число узлов).
...
def _load_nodes(store: Any) -> list[dict[str, Any]]:
    rows = store.rows(
        "MATCH (n:Node) WHERE n.name IS NOT NULL OR n.text IS NOT NULL "
        "OR n.canonical_name IS NOT NULL RETURN n",
        {},
    )
...
@router.get("/status")
def status() -> dict:
    """Готовность cross-lingual поиска: мультиязычная модель + число узлов графа."""
    store = get_store()
    ...
    return {
        "available": bool(matrix["items"]),
        "embedding_model": settings.embedding_model,
        "multilingual": "multilingual" in settings.embedding_model.lower(),
        "nodes_indexed": matrix["signature"],
