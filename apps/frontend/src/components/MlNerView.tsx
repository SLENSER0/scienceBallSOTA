import { useMemo, useState, type ReactNode } from 'react';
import { useMutation, useQuery } from '@tanstack/react-query';
import { BadgeCheck, BrainCircuit, CircleAlert, Loader2, Play, ScanText, TrendingUp } from 'lucide-react';

// §6.7 — Real GLiNER ML-NER (weights connected, not just rule-fallback).
// Self-contained (no api.ts edits): calls the /api/v1/ml-ner router directly with
// the same session-auth convention as api.ts.

function authHeaders(): Record<string, string> {
  try {
    const raw = localStorage.getItem('sb.session');
    if (raw) {
      const s = JSON.parse(raw);
      if (s?.token) return { Authorization: `Bearer ${s.token}` };
      if (s?.role) return { 'X-Role': s.role };
    }
  } catch {
    /* ignore */
  }
  return {};
}

async function apiGet<T>(url: string): Promise<T> {
  const res = await fetch(url, { headers: { ...authHeaders() } });
  if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);
  return res.json() as Promise<T>;
}

async function apiPost<T>(url: string, body: unknown): Promise<T> {
  const res = await fetch(url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...authHeaders() },
    body: JSON.stringify(body),
  });
  if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);
  return res.json() as Promise<T>;
}

interface Mention {
  label: string;
  text: string;
  char_start: number;
  char_end: number;
  score: number;
}
interface StatusResponse {
  gliner_available: boolean;
  backend: 'gliner' | 'rule';
  model: string | null;
  device: string;
  threshold: number;
  labels: string[];
  note: string;
}
interface ExtractResponse {
  backend: string;
  model: string | null;
  threshold: number;
  latency_ms: number;
  n_mentions: number;
  mentions: Mention[];
}
interface CorpusChunk {
  chunk_id: string;
  page: number | null;
  text: string;
  mentions: Mention[];
  n_mentions: number;
}
interface CorpusResponse {
  backend: string;
  model: string | null;
  threshold: number;
  chunks: CorpusChunk[];
  summary: {
    n_chunks: number;
    rule_mentions: number;
    ml_mentions: number;
    lift: number;
    lift_pct: number;
    latency_ms: number;
  };
  labels?: string[];
  note?: string;
}

// Stable colour per NER label so highlights read as one system.
const LABEL_HUES: Record<string, string> = {
  material: 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40',
  alloy: 'bg-teal-500/20 text-teal-300 border-teal-500/40',
  chemical_element: 'bg-cyan-500/20 text-cyan-300 border-cyan-500/40',
  sample: 'bg-sky-500/20 text-sky-300 border-sky-500/40',
  processing_operation: 'bg-amber-500/20 text-amber-300 border-amber-500/40',
  equipment: 'bg-orange-500/20 text-orange-300 border-orange-500/40',
  property: 'bg-violet-500/20 text-violet-300 border-violet-500/40',
  measurement: 'bg-fuchsia-500/20 text-fuchsia-300 border-fuchsia-500/40',
  method: 'bg-indigo-500/20 text-indigo-300 border-indigo-500/40',
  lab: 'bg-rose-500/20 text-rose-300 border-rose-500/40',
  person: 'bg-pink-500/20 text-pink-300 border-pink-500/40',
  research_team: 'bg-red-500/20 text-red-300 border-red-500/40',
};

function hueFor(label: string): string {
  return LABEL_HUES[label] ?? 'bg-slate-500/20 text-slate-300 border-slate-500/40';
}

const SAMPLE_TEXT =
  'Никелевый сплав ХН77ТЮР подвергали электроэкстракции в католите при температуре 60 °C ' +
  'и плотности тока 250 А/м². Твёрдость образца достигла 320 HV, предел прочности 780 МПа. ' +
  'Измерения выполнены на дифрактометре в лаборатории материаловедения.';

// Render a passage with its NER mentions highlighted inline, using char offsets.
function Highlighted({ text, mentions }: { text: string; mentions: Mention[] }) {
  const sorted = useMemo(
    () => [...mentions].sort((a, b) => a.char_start - b.char_start),
    [mentions],
  );
  const parts: ReactNode[] = [];
  let cursor = 0;
  sorted.forEach((m, i) => {
    if (m.char_start < cursor) return; // skip overlaps defensively
    if (m.char_start > cursor) parts.push(<span key={`t${i}`}>{text.slice(cursor, m.char_start)}</span>);
    parts.push(
      <span
        key={`m${i}`}
        title={`${m.label} · ${m.score.toFixed(2)}`}
        className={`rounded border px-1 ${hueFor(m.label)}`}
      >
        {text.slice(m.char_start, m.char_end)}
      </span>,
    );
    cursor = m.char_end;
  });
  if (cursor < text.length) parts.push(<span key="tail">{text.slice(cursor)}</span>);
  return <p className="text-sm leading-7 text-ink">{parts}</p>;
}

export function MlNerView() {
  const [text, setText] = useState(SAMPLE_TEXT);
  const [extractResult, setExtractResult] = useState<ExtractResponse | null>(null);
  const [corpus, setCorpus] = useState<CorpusResponse | null>(null);

  const status = useQuery({
    queryKey: ['ml-ner-status'],
    queryFn: () => apiGet<StatusResponse>('/api/v1/ml-ner/status'),
  });

  const runExtract = useMutation({
    mutationFn: () => apiPost<ExtractResponse>('/api/v1/ml-ner/extract', { text }),
    onSuccess: (d) => setExtractResult(d),
  });

  const runCorpus = useMutation({
    mutationFn: () => apiGet<CorpusResponse>('/api/v1/ml-ner/corpus?limit=12'),
    onSuccess: (d) => setCorpus(d),
  });

  const live = status.data;
  const isReal = live?.backend === 'gliner';

  return (
    <div className="h-full overflow-y-auto px-6 py-6">
      <div className="mx-auto max-w-5xl">
        <div className="eyebrow mb-1">ML-экстракция · §6.7</div>
        <h2 className="mb-1 font-display text-2xl font-semibold">Реальный GLiNER ML-NER</h2>
        <p className="mb-5 max-w-3xl text-sm text-faint">
          Гибкий zero-shot NER поверх подключённых весов GLiNER резко поднимает recall сущностей
          (материалы, сплавы, операции обработки, оборудование, свойства) сверх regex/словарей —
          граф становится плотнее и полнее. Когда веса недоступны, честно работает
          детерминированный rule-fallback.
        </p>

        {/* Backend status banner */}
        {live && (
          <div
            className={`panel mb-5 flex items-start gap-3 p-4 ${
              isReal ? 'border-emerald-500/40' : 'border-amber-500/40'
            }`}
          >
            {isReal ? (
              <BadgeCheck size={26} className="mt-0.5 shrink-0 text-emerald-400" />
            ) : (
              <CircleAlert size={26} className="mt-0.5 shrink-0 text-amber-400" />
            )}
            <div className="min-w-0">
              <div className="font-display text-ink">
                {isReal ? (
                  <>
                    Активны реальные ML-веса:{' '}
                    <span className="font-mono text-sm text-emerald-300">{live.model}</span>
                  </>
                ) : (
                  'Rule-fallback (regex/словари) — веса GLiNER не подключены'
                )}
              </div>
              <div className="mt-1 text-sm text-faint">{live.note}</div>
              <div className="mt-2 flex flex-wrap gap-2 text-xs">
                <span className="rounded bg-line/40 px-2 py-0.5 text-faint">
                  backend: <span className="font-mono text-ink">{live.backend}</span>
                </span>
                <span className="rounded bg-line/40 px-2 py-0.5 text-faint">
                  device: <span className="font-mono text-ink">{live.device}</span>
                </span>
                <span className="rounded bg-line/40 px-2 py-0.5 text-faint">
                  threshold: <span className="font-mono text-ink">{live.threshold}</span>
                </span>
                <span className="rounded bg-line/40 px-2 py-0.5 text-faint">
                  labels: <span className="font-mono text-ink">{live.labels.length}</span>
                </span>
              </div>
            </div>
          </div>
        )}
        {status.isError && (
          <div className="panel mb-5 border-red-500/40 p-3 text-sm text-red-400">
            Статус недоступен: {(status.error as Error).message}
          </div>
        )}

        {/* Domain labels */}
        {live && (
          <div className="mb-6 flex flex-wrap gap-1.5">
            {live.labels.map((l) => (
              <span key={l} className={`rounded border px-2 py-0.5 text-xs ${hueFor(l)}`}>
                {l}
              </span>
            ))}
          </div>
        )}

        {/* Ad-hoc extraction */}
        <div className="mb-8">
          <h3 className="mb-2 flex items-center gap-2 font-display text-lg">
            <ScanText size={18} className="text-copper" /> Разметка текста
          </h3>
          <textarea
            value={text}
            onChange={(e) => setText(e.target.value)}
            rows={4}
            className="mb-3 w-full rounded-lg border border-line bg-panel p-3 text-sm text-ink outline-none focus:border-copper"
            placeholder="Вставьте фрагмент текста для NER…"
          />
          <button
            onClick={() => runExtract.mutate()}
            disabled={runExtract.isPending || !text.trim()}
            className="btn-copper flex items-center gap-2"
          >
            {runExtract.isPending ? (
              <Loader2 size={16} className="animate-spin" />
            ) : (
              <BrainCircuit size={16} />
            )}
            {runExtract.isPending ? 'Извлечение…' : 'Извлечь сущности'}
          </button>

          {runExtract.isError && (
            <div className="panel mt-3 border-red-500/40 p-3 text-sm text-red-400">
              Ошибка: {(runExtract.error as Error).message}
            </div>
          )}

          {extractResult && (
            <div className="panel mt-4 p-4">
              <div className="mb-3 flex flex-wrap items-center gap-2 text-xs text-faint">
                <span className="rounded bg-line/40 px-2 py-0.5">
                  backend: <span className="font-mono text-ink">{extractResult.backend}</span>
                </span>
                <span className="rounded bg-line/40 px-2 py-0.5">
                  mentions: <span className="font-mono text-ink">{extractResult.n_mentions}</span>
                </span>
                <span className="rounded bg-line/40 px-2 py-0.5">
                  latency: <span className="font-mono text-ink">{extractResult.latency_ms} ms</span>
                </span>
              </div>
              <Highlighted text={text} mentions={extractResult.mentions} />
              {extractResult.mentions.length === 0 && (
                <p className="mt-2 text-sm text-faint">
                  Сущностей не найдено. С подключёнными весами GLiNER recall заметно выше.
                </p>
              )}
            </div>
          )}
        </div>

        {/* Corpus recall lift */}
        <div>
          <h3 className="mb-2 flex items-center gap-2 font-display text-lg">
            <TrendingUp size={18} className="text-copper" /> Recall-подъём на живом корпусе
          </h3>
          <p className="mb-3 max-w-3xl text-sm text-faint">
            Прогон батча реальных чанков из графа: сравниваем ML-NER с regex-базой на одних и тех
            же чанках и показываем, сколько дополнительных упоминаний даёт ML.
          </p>
          <button
            onClick={() => runCorpus.mutate()}
            disabled={runCorpus.isPending}
            className="btn-copper flex items-center gap-2"
          >
            {runCorpus.isPending ? (
              <Loader2 size={16} className="animate-spin" />
            ) : (
              <Play size={16} />
            )}
            {runCorpus.isPending ? 'Прогон корпуса…' : 'Прогнать батч чанков'}
          </button>

          {runCorpus.isError && (
            <div className="panel mt-3 border-red-500/40 p-3 text-sm text-red-400">
              Ошибка: {(runCorpus.error as Error).message}
            </div>
          )}

          {corpus && (
            <div className="mt-4 space-y-4">
              {corpus.note && (
                <div className="panel border-amber-500/40 p-3 text-sm text-amber-300">
                  {corpus.note}
                </div>
              )}
              {/* Lift stat row */}
              <div className="grid gap-3 sm:grid-cols-4">
                <div className="panel p-3">
                  <div className="text-xs uppercase text-faint">Чанков</div>
                  <div className="font-display text-2xl text-ink">{corpus.summary.n_chunks}</div>
                </div>
                <div className="panel p-3">
                  <div className="text-xs uppercase text-faint">Regex (rule)</div>
                  <div className="font-display text-2xl text-ink">
                    {corpus.summary.rule_mentions}
                  </div>
                </div>
                <div className="panel p-3">
                  <div className="text-xs uppercase text-faint">ML-NER</div>
                  <div className="font-display text-2xl text-emerald-300">
                    {corpus.summary.ml_mentions}
                  </div>
                </div>
                <div className="panel p-3">
                  <div className="text-xs uppercase text-faint">Подъём recall</div>
                  <div className="font-display text-2xl text-copper">
                    +{corpus.summary.lift}{' '}
                    <span className="text-sm text-faint">({corpus.summary.lift_pct}%)</span>
                  </div>
                </div>
              </div>
              <div className="text-xs text-faint">
                backend: <span className="font-mono text-ink">{corpus.backend}</span> · latency:{' '}
                <span className="font-mono text-ink">{corpus.summary.latency_ms} ms</span>
              </div>

              {/* Per-chunk highlights */}
              <div className="space-y-3">
                {corpus.chunks.map((c) => (
                  <div key={c.chunk_id} className="panel p-4">
                    <div className="mb-2 flex items-center gap-2 text-xs text-faint">
                      <span className="font-mono">{c.chunk_id}</span>
                      {c.page != null && <span>· стр. {c.page}</span>}
                      <span className="ml-auto rounded bg-line/40 px-2 py-0.5">
                        {c.n_mentions} упоминаний
                      </span>
                    </div>
                    <Highlighted text={c.text} mentions={c.mentions} />
                  </div>
                ))}
              </div>
            </div>
          )}
        </div>
      </div>
    </div>
  );
}
